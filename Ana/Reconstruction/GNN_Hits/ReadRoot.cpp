...
std::vector<Event> ReadRoot(const std::string &filename, float threshold = 0.0, float KNN_ratio = 0.5, int KNN_max = 16) {
    ...
    for (Long64_t i=0; i<nentries; ++i) {
        ...
        sort_event_by_z(ev);
        int KNN_K = std::min(KNN_max, std::max(2, (int)(ev.energy.size()*KNN_ratio)));
...
PYBIND11_MODULE(ReadRoot, m) {
    ...
    m.def("read_root", &ReadRoot, py::arg("filename"), py::arg("threshold")=0.0, py::arg("KNN_ratio")=0.5, py::arg("KNN_max")=16);
}
