...
double_t sum_energy_weighted_masking_notcorr(char* filename, int input_energy){

    //write the fit parameters to a txt file
    //for the e- files from fabricio
    ofstream myfile;
    myfile.open ("/home/llr/ilc/ritzmann/work/conf6_e-_GeV_5kevt_-42_-42_build_masked_params_sum_energy_weighted_masking_notcorr.txt", std::ios_base::app);
...
double ewesum_mask(float energy_sum){
    
    double val = e4*pow(energy_sum,4)+e3*pow(energy_sum,3)+e2*pow(energy_sum,2)+e1*energy_sum+e0;
    return val;
}
...
void sum_energy_weighted_masking_corr(char* filename, float input_energy, double a0, double a1, double a2, double a3){
    //write the fit parameters to a txt file
    //for the e- files from fabricio
    ofstream myfile;
    myfile.open ("/home/llr/ilc/ritzmann/work/conf6_e-_GeV_5kevt_-42_-42_build_masked_params_sum_energy_weighted_masking_corr.txt", std::ios_base::app);
    ...
    tree->Draw("ewesum_mask(Sum$((hit_isMasked==0)*hit_energy*f(hit_slab)))"); 
...
void sum_energy_weighted_masking_new(){
    ...
    myfile_events.open ("/home/llr/ilc/ritzmann/work/conf6_e-_GeV_5kevt_-42_-42_build_masked_params_sum_energy_weighted_masking_notcorr.txt");
    ...
    for (int i = 0; i < 11; i++) {
        ...
        double_t mean_val = sum_energy_weighted_masking_notcorr((char*)argument.c_str(), energies[i]);
    ...
    e0 = fit_pol->GetParameter(0);
    e1 = fit_pol->GetParameter(1);
    e2 = fit_pol->GetParameter(2);
    e3 = fit_pol->GetParameter(3);
    e4 = fit_pol->GetParameter(4);
    ...
    myfile_events2.open ("/home/llr/ilc/ritzmann/work/conf6_e-_GeV_5kevt_-42_-42_build_masked_params_sum_energy_weighted_masking_corr.txt");
    ...
    for (int i = 0; i < 11; i++) {
        ...
        sum_energy_weighted_masking_corr((char*)argument.c_str(), energies[i], e0, e1, e2, e3);
