...
void sum_energy_weighted_notcorr(char* filepath, float input_energy, double_t &mean, double_t &error_val, ofstream &file_notcorr);
void sum_energy_weighted_corr(char* filepath, TSpline3* spline_fit, float input_energy, ofstream &mean_file, ofstream &median_file, ofstream &gauss_file, ofstream &gauss_file2s, ofstream &rms68_file, ofstream &rms90_file, ofstream &myfile_3sig, ofstream &myfile_3sig_fit, ofstream &gamma_file);
void calc_median__(TTree *tree, TH1F *h, TSpline3* spline_fit, ofstream &median_file, float input_energy);

void sum_energy_weighted(){
    ...
    file_notcorr.open ("/home/llr/ilc/ritzmann/work/res_csv_files/conf6_e-_GeV_5kevt_-42_-42_build_masked_params_sum_energy_weighted_mean_test.txt");
    ...
    for(int i = 0; i < 16; i++) {
        ...
        sum_energy_weighted_notcorr((char*)argument.c_str(), energies[i], mean_val, error_val, file_notcorr);
    ...
    ofstream mean_file;
    mean_file.open ("/home/llr/ilc/ritzmann/work/res_csv_files/conf6_e-_GeV_5kevt_-42_-42_build_masked_params_sum_energy_weighted_corr_mean_test.txt");
    mean_file << "energy,mean,sigma" << "\n";

    ofstream median_file;
    median_file.open ("/home/llr/ilc/ritzmann/work/res_csv_files/conf6_e-_GeV_5kevt_-42_-42_build_masked_params_sum_energy_weighted_corr_median_test.txt");
    median_file << "energy,mean,sigma" << "\n";

    ofstream gauss_file;
    gauss_file.open ("/home/llr/ilc/ritzmann/work/res_csv_files/conf6_e-_GeV_5kevt_-42_-42_build_masked_params_sum_energy_weighted_corr_gauss_test.txt");
    gauss_file << "energy,mean,mean_err,sigma,sigma_err,goodness" << "\n";

    ofstream gauss_file2s;
    gauss_file2s.open ("/home/llr/ilc/ritzmann/work/res_csv_files/conf6_e-_GeV_5kevt_-42_-42_build_masked_params_sum_energy_weighted_corr_gauss2s_test.txt");
    gauss_file2s << "energy,mean,mean_err,sigma,sigma_err,goodness" << "\n";

    ofstream rms68_file;
    rms68_file.open ("/home/llr/ilc/ritzmann/work/res_csv_files/conf6_e-_GeV_5kevt_-42_-42_build_masked_params_sum_energy_weighted_corr_rms68_test.txt");
    rms68_file << "energy,mean,sigma" << "\n";

    ofstream rms90_file;
    rms90_file.open ("/home/llr/ilc/ritzmann/work/res_csv_files/conf6_e-_GeV_5kevt_-42_-42_build_masked_params_sum_energy_weighted_corr_rms90_test.txt");
    rms90_file << "energy,mean,sigma" << "\n";

    ofstream myfile_3sig;
    myfile_3sig.open("/home/llr/ilc/ritzmann/work/res_csv_files/conf6_e-_GeV_5kevt_-42_-42_build_masked_ratio_3sigmas_sum_energy_weighted_test.txt");
    myfile_3sig << "energy,ratio" << "\n";

    ofstream myfile_3sig_fit;
    myfile_3sig_fit.open("/home/llr/ilc/ritzmann/work/res_csv_files/conf6_e-_GeV_5kevt_-42_-42_build_masked_ratio_3sigmas_sum_energy_weighted_fitted_test.txt");
    myfile_3sig_fit << "energy,ratio" << "\n";

    ofstream gamma_file;
    gamma_file.open ("/home/llr/ilc/ritzmann/work/res_csv_files/conf6_e-_GeV_5kevt_-42_-42_build_masked_params_sum_energy_weighted_corr_gamma_test.txt");
    ...
    for(int i = 0; i < 16; i++) {
        ...
        sum_energy_weighted_corr((char*)argument.c_str(), spline_fit, energies[i], mean_file, median_file, gauss_file, gauss_file2s, rms68_file, rms90_file, myfile_3sig, myfile_3sig_fit, gamma_file);
...
void sum_energy_weighted_notcorr(char* filepath, float input_energy, double_t &mean, double_t &error_val, ofstream &file_notcorr){
    ...
    TH1F* hist = new TH1F(("hits" + to_string(input_energy)).c_str(), to_string(input_energy).c_str(), 100 , xmin, xmax);
    ...
    for (int i = 0; i < entries; i++) {
        ...
        for (int j = 0; j < n_hit; j++) {
            int layer = *slice;
            tot_energy = tot_energy + *energy * f(layer);
            sumE = sumE + *energy;
            energy++;
            slice++;
        }
...
void sum_energy_weighted_corr(char* filepath, TSpline3* spline_fit, float input_energy, ofstream &mean_file, ofstream &median_file, ofstream &gauss_file, ofstream &gauss_file2s, ofstream &rms68_file, ofstream &rms90_file, ofstream &myfile_3sig, ofstream &myfile_3sig_fit, ofstream &gamma_file){
    ...
    int nbins = ceil((xmax-xmin)/bin_width);
    cout << bin_width << endl;

    TH1F* hist_corr = new TH1F(("corr_hits" + to_string(input_energy)).c_str(), to_string(input_energy).c_str(), 100 , xmin, xmax);
    ...
    calc_median__(tree, hist_corr, spline_fit, median_file, input_energy);
    ...
    void calc_median__(TTree *tree, TH1F *h, TSpline3* spline_fit, ofstream &median_file, float input_energy){
        ...
        for (int i = 0; i < entries; i++) {
            ...
            for (int j = 0; j < n_hit; j++) {
                int layer = *slice;
                tot_energy = tot_energy + *energy * f(*slice);
                energy++;
                slice++;
            }
